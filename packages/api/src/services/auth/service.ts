...
export default class AuthService {
  ...
  /**
   * Ends the session server-side. The caller still forgets the stored credential itself, since a failed request must not leave the user signed in.
   *
  ...
  /**
   * Exchanges the code OIDC sent the user back with for a session. The code is single use, so this runs once per redirect.
   *
